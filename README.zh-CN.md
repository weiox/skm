...
`skm` 主要就是为了解决下面这些反复出现的痛点：

- **“我看到一个 skill 包，但不知道到底该放到哪里。”**
  很常见的做法是直接把 skill clone 或复制到 `~/.agents/skills` 或 `~/.claude/skills`。一开始看起来能用，但之后会越来越难更新、追踪来源和迁移到新机器。`skm` 的做法是把外部 skill 包统一收进 `vendor/`，再从受管理的 source tree 重建入口层。

- **“我的本地 skills 已经散得到处都是。”**
  时间一长，skills 往往会散落在工具入口目录、个人目录、旧机器迁移目录和各种临时实验目录中。到了这个阶段，你已经很难判断哪一份才是真正的 source of truth。`skm` 用 `~/.skm` 给你一个统一声明态，并把 agent 可见目录视为生成出来的入口层，而不是可直接维护的源码目录。

- **“Codex 能看到，Claude Code 却看不到。”**
  这通常意味着入口层已经漂移：旧 symlink 还挂着、手工加过 unmanaged 链接，或者两个 agent 的可见层已经不一致。`skm` 提供的是先诊断、后修复的流程，让你先看清哪些是 `OK`、`BROKEN`、`UNMANAGED`，再决定是否重建运行态。

- **“我想更新 vendor skills，但不敢手工改。”**
  在错误的位置直接 `pull`，或者手工改 symlink 入口层，风险很大，而且很难重复。`skm` 把 vendor 更新变成受管理流程：更新包、重建入口、再验证结果。

- **“我写了一些有用的本地 skills，但没有一条干净的发布路径。”**
  很多 personal skills 一开始只是本地实验，后来逐渐变得可复用。如果没有结构，把它们拆成独立 skill 包会很混乱。`skm` 提供了一条从本地 skills 到可抽取、可发布 skill 包的路径，同时又不打破本地 source-of-truth 模型。
